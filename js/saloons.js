export default class SaloonPage {
  ...
  addSeatRowActive(seatCounter) {
    ...
    // Testing to get info and save from a json file
  
    async read() {
    // Read data from a JSON file
    this.testSave = await JSON._load('../js/saloons/testSave.json');
    console.log('Read');
    console.table(this.testSave);
    this.updateSeats()    
  }

  async updateSeats() {

    console.log('TestSave' + this.testSave)
    /*let name = 'ulf';
    // Add the person to the list
    this.testSave.push({ name }); // (short for {name: name})
    // Save the data to a JSON file
    await JSON._save('../js/saloons/testSave.json', this.testSave);
    console.log('Saved');
    console.table(this.testSave);*/
  }
